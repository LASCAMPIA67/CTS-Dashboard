...
async function enrichSlices(text, slices) {
  return Promise.all(
    slices.map(async slice => {
      const startsAtDepot =
        await DB.isDepot(slice.startPlaceCode)

      const details =
        await extractDepartureDetails(
          text,
          slice,
          startsAtDepot
        )
  ...
  contexts.forEach(
    (
      ...
      if (
        ...
        warnings.push(
          `Heure de sortie dépôt introuvable pour la tranche ${number}`
        )
      }
      ...
      if (
        ...
        warnings.push(
          `Direction introuvable pour la tranche ${number}`
        )
      }
    }
  )
...
function extractDriver(text) {
  const match =
    text.match(
      /\b([A-ZÀ-ÖØ-Ý][A-ZÀ-ÖØ-Ý' -]{3,})\s*\((\d{4,10})\)/
    )
...
async function extractSlices(text) {
  ...
  return removeDuplicateSlices(
    slices
  )
}

async function extractDepartureDetails(
  text,
  slice,
  startsAtDepot
) {
...
function extractVehicleSection(
  text,
  slice
) {
  const header =
    new RegExp(
      `\\bVoiture\\s+${escapeRegex(
        slice.lineCode
      )}\\s*-\\s*${escapeRegex(
        String(
          slice.vehicle
  ...
  const nextHeader =
    /\bVoiture\s+\d{1,3}\s*-\s*\d{1,3}\b/i.exec(
      rest
    )
...
async function extractLineUpPlace(
  section,
  lines,
  slice
) {
  ...
  if (
    ...
    return (
      await extractTramOperationStart(
        lines,
        slice
      )
    )
  ...
  const firstRegular =
    flatSection.match(
      /Régulier\s*\/\s*\S+\s+(.+?)\s+\d{1,2}:\d{2}/i
    )
...
async function extractTramOperationStart(
  lines,
  slice
) {
  const exitIndex =
...
function isTramLineCode(
  value
) {
  return TRAM_LINE_CODES.has(
    normalizeCode(
      value
    )
  )
...
async function extractFirstDirection(
  section,
  lines
) {
  const regularIndex =
  ...
  return (
    extractDirectionFromFlatText(
      section
    )
  )
}

async function extractDirectionFromFlatText(
  section
) {
  const flatText =
    section
  ...
  const matches = [
    ...block.matchAll(
      /(?:^|\s)([A-ZÀ-ÖØ-Ý][A-ZÀ-ÖØ-Ý0-9' ().-]*?)\s+(\d{1,2}:\d{2})(?=\s|$)/g
    )
  ]
...
function extractTimedStop(
  ...
  if (
    /^(Prép\.\s*sortie|Déplacement|Pause-café|Coupure)$/i.test(
      name
    )
  ) {
    return null
...
function isActivityLine(
  line
) {
  return (
    /^(Régulier|Haut-le-pied|Entrée|Sortie)\s*\//i.test(
      line
    )
  )
}

function isDirectionBoundary(
  line
) {
  return (
    isActivityLine(
    ...
    /^(Déplacement|Coupure|Pause-café|Prép\.\s*sortie)\b/i.test(
      line
    )
  )
}
...
function extractBreaks(
  ...
  const regex =
    /\bCoupure\s+(\d{1,2}:\d{2})\s+(\d{1,2}:\d{2})\b/gi
...
function validateSlices(
  ...
  slices.forEach(
    (
      ...
      if (
        ...
        errors.push(
          `Ligne absente pour la tranche ${number}`
        )
      }
      ...
      if (
        ...
        errors.push(
          `Voiture absente pour la tranche ${number}`
        )
      }
      ...
      if (
        ...
        errors.push(
          `Horaires invalides pour la tranche ${number}`
        )
      }
      ...
      if (
        ...
        errors.push(
          `Fin antérieure au début pour la tranche ${number}`
        )
      }
      ...
      if (
        ...
        warnings.push(
          `Lieu inconnu dans la tranche ${number}`
        )
      }
    }
  )
  ...
  for (
    ...
    if (
      ...
      warnings.push(
        `Chevauchement entre les tranches ${index} et ${index + 1}`
      )
    }
  }
}

function removeDuplicateSlices(
  slices
) {
  const seen =
    new Set()
