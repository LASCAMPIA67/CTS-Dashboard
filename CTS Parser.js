...
async function extractDepartureDetails(text, slice, startsAtDepot) {
  ...
  return {
    depotExitAt: startsAtDepot
      ? extractDepotExitTime(section, lines)
      : "",

    lineUpAt: startsAtDepot
      ? await extractLineUpPlace(section, lines)
      : "",
...
function extractDepotExitTime(section, lines) {
  const exitLine = lines.find(line =>
    /^Sortie\s*\/\s*-/i.test(line)
  )

  if (exitLine) {
    const match = exitLine.match(
      /^Sortie\s*\/\s*-\s+.+?\s+(\d{1,2}:\d{2})$/i
    )
  ...
  const match = flatSection.match(
    /\bSortie\s*\/\s*-\s+DEPOT\s+.+?\s+(\d{1,2}:\d{2})(?=\s|$)/i
  )

  return match && isValidTime(match[1])
    ? normalizeTime(match[1])
    : ""
}
...
function extractVehicleSection(text, slice) {
  ...
  const nextHeader =
    /\bVoiture\s+\d{1,3}\s*-\s*\d{1,3}\b/i.exec(rest)
...
async function extractLineUpPlace(section, lines) {
  const exitIndex = lines.findIndex(line =>
    /^Sortie\s*\/\s*-/i.test(line)
  )
  ...
  if (exitIndex !== -1) {
    for (let index = exitIndex + 1; index < lines.length; index++) {
      ...
      if (isActivityLine(line)) break

      const stop = extractTimedStop(line)
      if (stop) return DB.formatStop(stop.name)
    }
  }
  ...
  const exitMatch = flatSection.match(
    /Sortie\s*\/\s*-\s+DEPOT\s+.+?\s+\d{1,2}:\d{2}\s+-\s*\/\s*-\s+(.+?)\s+\d{1,2}:\d{2}\s+Régulier\s*\//i
  )

  if (exitMatch) return DB.formatStop(exitMatch[1])

  const firstRegular = flatSection.match(
    /Régulier\s*\/\s*\S+\s+(.+?)\s+\d{1,2}:\d{2}/i
  )

  return firstRegular
    ? DB.formatStop(firstRegular[1])
    : ""
}

async function extractFirstDirection(section, lines) {
  const regularIndex = lines.findIndex(line =>
    /^Régulier\s*\/\s*\S+/i.test(line)
  )

  if (regularIndex !== -1) {
    const departure = extractActivityStop(lines[regularIndex])
    let direction = departure
      ? await DB.formatStop(departure.name)
      : ""

    for (
      let index = regularIndex + 1;
      index < lines.length;
      index++
    ) {
      const line = lines[index]

      if (isDirectionBoundary(line)) break

      const stop = extractTimedStop(line)
      if (stop) direction = await DB.formatStop(stop.name)
    }
...
async function extractDirectionFromFlatText(section) {
  const flatText = section
    .replace(/\n/g, " ")
    .replace(/\s+/g, " ")
    .trim()

  const start = flatText.search(
    /\bRégulier\s*\/\s*\S+/i
  )
  ...
  const block = nextBlock === -1
    ? afterStart
    : afterStart.slice(0, nextBlock + 1)
...
function extractTimedStop(line) {
  ...
  if (
    /^(Prép\.\s*sortie|Déplacement|Pause-café|Coupure)$/i.test(name)
  ) {
    return null
  }
...
function isActivityLine(line) {
  return /^(Régulier|Haut-le-pied|Entrée|Sortie)\s*\//i.test(
    line
  )
}
    ...
    const alreadyCut = breaks.some(
      interruption =>
        ...
        (
          ...
          interruption.end === next.dutyStart
        )
    )
...
function validateSlices(slices, errors, warnings) {
  ...
  for (let index = 1; index < slices.length; index++) {
    if (
      toMinutes(slices[index].operationStart) <
      toMinutes(slices[index - 1].end)
    ) {
      warnings.push(
