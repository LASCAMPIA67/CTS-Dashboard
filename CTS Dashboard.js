...
async function main() {
  ...
  try {
    await displayWidget(widget, family, sendTelemetrySafely(analytics, telemetryRun))
  } catch (error) {
    console.warn("[Dashboard]", UTILS.errorMessage(error))
...
function recordRunTrace(context) {
  runTrace = {
    ...
    displayed: context?.valid ? "service" : String(context?.errorTitle || "inconnu"),
...
function writeRunTrace() {
  try {
    ...
    fm.writeString(fm.joinPath(CONFIG.paths.data, "last-run.json"), JSON.stringify(runTrace))
  } catch (error) {
    console.warn("[Dashboard]", UTILS.errorMessage(error))
  }
}
...
async function displayWidget(widget, widgetFamily, telemetryPromise) {
  if (config.runsInWidget) {
    ...
    await Promise.race([telemetryPromise, UTILS.sleep(WIDGET_COMMIT_YIELD_MS)])
